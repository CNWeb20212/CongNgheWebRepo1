<?php
	class friendcontroller{
		...
		//khi mình xóa lời mời kết bạn của mình
		public function deletemyrequest($receiver){
			$friendInstance = new friend();
			$res = $friendInstance->deletemyrequest($receiver);
			if($res){
				header('location:/BKSNet/friendcontroller/viewuser/'.$receiver);
			} else{
				?>
				<script type="text/javascript">
					alert("Hủy lời mời kết bạn thất bại, vui lòng thử lại");
				</script>
				<?php
			}
		}
